...
class DbOperation{
    ...
    //method for getting phone number alarm where status=true
    public function getalarm_isOn_Locid_Loc_Mgr(){
        $status=1;
         $stmt= $this->con->prepare("SELECT location.Loc_ID, location.Loc_Mgr_ID,user.USR_Contact FROM location INNER JOIN user ON location.Loc_Mgr_ID=user.USR_ID WHERE location.Loc_Alarm_status =?");
    ...
    //method for set alarm status
    public function setalarmstatus($status,$locid){
        
        //creating statement
        $stmt= $this->con->prepare("UPDATE location SET Loc_Alarm_status=? WHERE Loc_ID=?");
        
        //binding values
        $stmt->bind_param("ss",$status,$locid);
    ...
    public function pushdata($locid,$valtemp,$valhumidity,$date,$time){
        
        //creating statement
        $stmt= $this->con->prepare("INSERT INTO data(Loc_ID,Val_Temp,Val_Humidity,date,time) values(?,?,?,?,?)");
        
        //binding values
        $stmt->bind_param("sssss",$locid,$valtemp,$valhumidity,$date,$time);
    ...
    public function getStatus($locationId){
        $stmt = $this->con->prepare("SELECT * FROM location WHERE Loc_id=?");
    ...
    public function isValidUser($key) {
        ...
        $stmt = $this->con->prepare("SELECT USR_ID from user WHERE USR_Key = ?");
