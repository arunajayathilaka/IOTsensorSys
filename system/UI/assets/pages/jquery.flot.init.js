...
*/
	...
	/*
		...
		*/
		...
		var plot = this.createRealTimeGraph('#flotRealTime', this.randomData(), ['#4285F4']);
		plot.draw();
		var $this = this;
		function updatePlot() {
		...
		var plot2 = this.createRealTimeGraph('#flotRealTime2', this.randomData(), ['#5fbeaa']);
		plot2.draw();
		var $this = this;
		function updatePlot2() {
			plot2.setData([$this.randomData()]);
			// Since the axes don't change, we don't need to call plot.setupGrid()
			plot2.draw();

			setTimeout(updatePlot2, $('html').hasClass('mobile-device') ? 1000 : 1000);
		}

		updatePlot2();

		var plot3 = this.createRealTimeGraph('#flotRealTime3', this.randomData(), ['#4285F4']);
		plot3.draw();
		var $this = this;
		function updatePlot3() {
			plot3.setData([$this.randomData()]);
			// Since the axes don't change, we don't need to call plot.setupGrid()
			plot3.draw();

			setTimeout(updatePlot3, $('html').hasClass('mobile-device') ? 1000 : 1000);
		}

		updatePlot3();

		var plot4 = this.createRealTimeGraph('#flotRealTime4', this.randomData(), ['#5fbeaa']);
		plot4.draw();
		var $this = this;
		function updatePlot4() {
			plot4.setData([$this.randomData()]);
			// Since the axes don't change, we don't need to call plot.setupGrid()
			plot4.draw();

			setTimeout(updatePlot4, $('html').hasClass('mobile-device') ? 1000 : 1000);
		}
